     ...
     return (
          <div className='w-full'>
               ...
               <div className='text-center space-y-3'>
                    <div className='text-5xl font-bold'>PDF to JPG</div>
                    <div className='text-2xl font-medium'>Convert each PDF page into a JPG or extract all images contained in a PDF.</div>
               </div>

               <div className='flex'>
                    <div className={selectedPage.length > 0 ? "w-3/4" : "w-full"}>
                         {selectedFiles.length === 0 && (
                              <div className="mt-10 mx-auto flex items-center justify-center w-1/2 bg-[#e5322d] rounded-lg cursor-pointer">
                         ...
                         {images.length > 0 && (
                              <>
                                   <div className='text-lg font-thin text-gray-700 text-center'>Select image which you wnat to convert into image</div>
                                   <div className='flex flex-wrap gap-5 max-h-[80vh] p-2 mx-auto w-[95%] overflow-y-scroll mt-10 mr-5'>
                                        {images.map((imageUrl, index) => (
                                             <div key={index} onClick={(e) => handlePageSelection(index, imageUrl, e)} className={`p-8 border h-fit rounded-md border-red-500 hover:shadow-xl ${pageDelete.includes(index) ? 'opacity-40' : ''}`}>
                                                  <div className='mx-auto'>
                                                       <img draggable={true} className={`shadow-[1px_1px_10px_black] rounded-lg h-48 cursor-move ${selectedPage.length > 0 ? "w-32" : "w-36"}`} src={imageUrl} alt={`Page ${index + 1}`} />
                    ...
                    {selectedPage.length > 0 && (
                         <>
                              ...
                              <div className='text-lg text-gray-500 w-96 overflow-x-scroll'>Page Slected : {pageDelete.join(" , ")}</div>
                                   <div onClick={handleDownloadImage} className='flex flex-col cursor-pointer place-content-center items-center bg-[#e5322d] w-fit mt-5 text-white font-semibold text-xl px-6 py-2 rounded-md active:opacity-70 mx-auto'>
                                        ...
                                        <div>Convert Selected Page As Image</div>
                                   </div>
                              </div>
                         </>
                    )}
