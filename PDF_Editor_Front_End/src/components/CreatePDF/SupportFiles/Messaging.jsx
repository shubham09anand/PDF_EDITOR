...
const Messaging = ({editorHeight}) => {
     ...
     return (
          <div style={{ height: `${editorHeight + 20}px`}} className={`w-full mt-16 shadow-[2px_2px_2px_2px_gray] bg-white absolute right-0 -top-2 overflow-y-hidden z-20`}>
                              ...
                              <div className='h-[560px] lg:h-[600px] pb-20 overflow-y-scroll example space-y-3'>
                                   {mettingMess && mettingMess.length >= 1 && mettingMess.map((mess, index) => (
                                        <div key={index} className='mb-2'>
                                             ...
                                             <div className="px-4 py-2 rounded-2xl w-fit max-w-44 lg:max-w-full bg-blue-500 text-white"> {mess?.message} </div>
                                        </div>
                                   ))}
